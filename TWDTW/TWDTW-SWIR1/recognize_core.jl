...
# VERSION: v2021.12.14
#=
Identification and classification based on statistical area setting thresholds (core function)
使用面积卡阈值识别分类（核心函数）
=#
...
function recognize(dist, ref, croparea, validmax=nothing; proj, rev=false, mergesort=true)
    ...
    elseif proj == "WGS84"
        ...
        if sum(valid0) < maxnum error("cropland area not enough") end
        # 大于（或小于，若rev 为 true） validmax 的像元不参与排序，节省计算时间
        if isnothing(validmax)
            valid = valid0
        else
            if rev
                valid = dist .> validmax
                while sum(valid) < maxnum # 自适应扩大
